...
const EditProfile: React.FC<Props> = ({ isOpen, onClose, user }) => {
  ...
  const handleFileChane = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const onSubmit = async (data: User) => {
    if (id) {
      try {
        ...
        data.dateOfBirth && data.dateOfBirth !== user?.dateOfBirth && formData.append('dateOfBirth', new Date(data.dateOfBirth).toISOString())
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className={`${theme} text-foregroud`}>
      <ModalContent>
        {
          (onClose) => (
            <>
